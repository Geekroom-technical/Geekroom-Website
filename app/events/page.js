          ...
          <div className="grid grid-cols-1 md:grid-cols-1 gap-12 w-full max-w-4xl">
            {events.map((event, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className={`group relative bg-white/40 backdrop-blur-2xl border-[1.5px] border-white/50 rounded-[2.5rem] p-10 shadow-sm transition-all duration-500 text-left ${executed ? 'opacity-60 grayscale-[0.5] hover:grayscale-0' : ongoing ? 'border-orange-500/30 bg-orange-50/10' : 'hover:bg-white hover:scale-[1.01] hover:shadow-2xl hover:shadow-black/5'}`}
                >
                  <div className={`absolute -top-4 -left-4 w-12 h-12 flex items-center justify-center rounded-2xl rotate-[-12deg] shadow-lg font-bold text-xl group-hover:rotate-0 transition-transform duration-300 ${executed ? 'bg-gray-400 text-white' : ongoing ? 'bg-orange-500 text-white animate-bounce' : 'bg-black text-white'}`}>
                    {idx + 1}
                  </div>
                  ...
                  <div className="mt-10">
                    {executed ? (
                      ...
                      <div className="inline-flex items-center gap-3">
                        <span className="text-[10px] font-black text-orange-600 uppercase tracking-widest border-b-2 border-orange-500 pb-1">Ongoing Now</span>
