        ...
        {events.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-1 gap-12 w-full max-w-4xl">
            {events.map((event, idx) => (
              <div 
                key={idx} 
                className="group relative bg-white/40 backdrop-blur-2xl border-[1.5px] border-white/50 rounded-[2.5rem] p-10 shadow-sm transition-all duration-500 hover:bg-white hover:scale-[1.01] hover:shadow-2xl hover:shadow-black/5 text-left"
              >
                <div className="absolute -top-4 -left-4 w-12 h-12 bg-black text-white flex items-center justify-center rounded-2xl rotate-[-12deg] shadow-lg font-bold text-xl group-hover:rotate-0 transition-transform duration-300">
                  {idx + 1}
                </div>

                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                  <h4 className="text-3xl sm:text-4xl font-bold text-[#1a1a1a] leading-none tracking-tighter">
                    {event.title}
                  </h4>
                  <div className="shrink-0 inline-flex items-center bg-[#f0f0f0] border border-black/5 text-black text-[10px] font-bold px-4 py-2 rounded-full tracking-widest shadow-inner">
                ...
                <p className="text-gray-800 text-lg leading-relaxed font-semibold pr-8">
                  {event.description}
                </p>
