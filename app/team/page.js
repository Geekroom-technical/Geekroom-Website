        ...
        {teamData?.teams && (
          <div className="w-full space-y-48 pb-20">
            {teamData.teams.map((team, tIdx) => {
              // Determine if we should span full width due to lack of other members
              const hasNoSideMembers = !team.colead && (!team.members || team.members.length === 0);
              const leadershipCount = (team.lead ? 1 : 0) + (team.core_members?.length || 0);
              const hasMultipleLeaders = leadershipCount > 1;

              return (
                <div key={tIdx} className="w-full space-y-12">
                  <div className="bg-black text-white px-8 py-2 rounded-full inline-block text-[10px] font-bold tracking-[0.4em] uppercase">
                    {team.name}
                  </div>

                  <div className={`grid grid-cols-1 ${hasNoSideMembers ? "flex flex-col items-center justify-center text-center" : "lg:grid-cols-3 text-left"} gap-12 items-start`}>
                    {/* Leadership Column (Lead & Core Members) */}
                    <div className={`${hasNoSideMembers ? (leadershipCount === 1 ? "max-w-xs mx-auto" : "w-full") : "lg:col-span-1"}`}>
                      <div className={`grid grid-cols-1 ${hasMultipleLeaders ? "md:grid-cols-2" : ""} gap-8`}>
                        {/* Render Lead if exists */}
                        {team.lead && (
                          <div className="group relative bg-white border-2 border-black rounded-[2.5rem] p-10 transition-all duration-500 hover:shadow-2xl hover:shadow-black/5 hover:-translate-y-1">
                            {team.lead.image && (
                              <div className="w-full aspect-square rounded-2xl overflow-hidden mb-8 border-2 border-black/5 grayscale group-hover:grayscale-0 transition-all duration-700">
                                 <Image 
                                   src={team.lead.image} 
                                   alt={team.lead.name}
                                   width={400}
                            ...
                            <div className="space-y-2">
                              <p className="text-xs font-bold text-gray-400 uppercase tracking-widest leading-none mb-2">Lead:</p>
                              <h4 className="text-2xl font-bold text-black tracking-tighter leading-none">{team.lead.name}</h4>
                              <p className="text-xs font-semibold text-gray-500">{team.lead.role}</p>
                              {team.lead.tagline && (
                                <p className="text-xs italic text-gray-500 mt-3 font-medium opacity-90 leading-relaxed">"{team.lead.tagline}"</p>
                              )}
                            </div>
                            {team.lead.github && (
                              <a href={team.lead.github} target="_blank" className="mt-8 inline-flex p-3 bg-gray-50 rounded-xl hover:bg-black hover:text-white transition-all">
                        ...
                        {/* Render Core Members horizontally if multiple */}
                        {team.core_members?.map((core, cIdx) => (
                          <div key={cIdx} className="group relative bg-white border-2 border-black rounded-[2.5rem] p-10 transition-all duration-500 hover:shadow-2xl hover:shadow-black/5 hover:-translate-y-1">
                            {core.image && (
                              <div className="w-full aspect-square rounded-2xl overflow-hidden mb-8 border-2 border-black/5 grayscale group-hover:grayscale-0 transition-all duration-700">
                                 <Image 
                                   src={core.image} 
                            ...
                            <div className="space-y-2">
                              <p className="text-xs font-bold text-gray-400 uppercase tracking-widest leading-none mb-2">Core Member:</p>
                              <h4 className="text-2xl font-bold text-black tracking-tighter leading-none">{core.name}</h4>
                              <p className="text-xs font-semibold text-gray-500">{core.role}</p>
                              {core.tagline && (
                                <p className="text-xs italic text-gray-500 mt-3 font-medium opacity-90 leading-relaxed">"{core.tagline}"</p>
                              )}
                            </div>
                          </div>
                        ))}
                    ...
                    {/* Member & Co-Lead Grid (only if side members exist) */}
                    {!hasNoSideMembers && (
                      <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4 h-fit">
                        {/* Co-Lead Card in the Grid */}
                        {team.colead && (
                          <div className="bg-white/40 backdrop-blur-md border border-black/5 rounded-[2rem] p-8 flex flex-col justify-between transition-all hover:bg-white hover:shadow-xl group/card">
                            <div className="space-y-1">
                              <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest leading-none mb-2">Co-Lead</p>
                              <h5 className="text-xl font-bold text-black tracking-tight">{team.colead.name}</h5>
                              <p className="text-xs font-semibold text-gray-500 leading-tight">{team.colead.role}</p>
                            </div>
                            {team.colead.github && (
                              <a href={team.colead.github} target="_blank" className="mt-6 opacity-0 group-hover/card:opacity-100 transition-opacity">
                        ...
                        {/* Regular Members in the Grid */}
                        {team.members?.map((member, mIdx) => (
                          <div key={mIdx} className="bg-white/40 backdrop-blur-md border border-black/5 rounded-[2rem] p-8 flex flex-col justify-between transition-all hover:bg-white hover:shadow-xl group/item">
                            <div className="space-y-1">
